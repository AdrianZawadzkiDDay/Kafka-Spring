...
class KafkaConsumerConfiguration {
    ...
    public ConcurrentKafkaListenerContainerFactory<String, MessageReadEvent> kafkaListenerContainerFactory(OffsetRepository offsetRepository, StoreOffsetRegistry storeOffsetRegistry) {
        var factory = new ConcurrentKafkaListenerContainerFactory<String, MessageReadEvent>();
        factory.setConsumerFactory(consumerFactory());
        factory.getContainerProperties().setSyncCommits(true);
        factory.setBatchListener(true);
        factory.getContainerProperties().setAckMode(ContainerProperties.AckMode.MANUAL_IMMEDIATE);
        factory.getContainerProperties().setConsumerRebalanceListener(new RebalanceListener(offsetRepository, storeOffsetRegistry));
        return factory;
    }
...
//        properties.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, JsonDeserializer.class);
        ...
        properties.putAll(kafkaProperties.getSafeProperties());
